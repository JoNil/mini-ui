...
use crate::cairo::{ffi, utils::status_to_result, Context, Error, Glyph};

type BoxInitFunc =
    Box<dyn Fn(&ScaledFont, &Context, &mut FontExtents) -> Result<(), Error> + Send + Sync>;
type BoxRenderGlyphFunc = Box<
    dyn Fn(&ScaledFont, std::ffi::c_ulong, &Context, &mut TextExtents) -> Result<(), Error>
        + Send
        + Sync,
>;
type BoxUnicodeToGlyphFunc =
    Box<dyn Fn(&ScaledFont, std::ffi::c_ulong) -> Result<std::ffi::c_ulong, Error> + Send + Sync>;
...
impl UserFontFace {
    ...
    where
        F: Fn(&ScaledFont, std::ffi::c_ulong, &Context, &mut TextExtents) -> Result<(), Error>
            + Send
            + Sync
            + 'static,
    {
    ...
    where
        F: Fn(&ScaledFont, std::ffi::c_ulong) -> Result<std::ffi::c_ulong, Error>
            + Send
            + Sync
            + 'static,
    {
        ...
        unsafe extern "C" fn unicode_to_glyph_trampoline(
            scaled_font: *mut ffi::cairo_scaled_font_t,
            unicode: std::ffi::c_ulong,
            glyph_index: *mut std::ffi::c_ulong,
        ) -> ffi::cairo_status_t {
            ...
            + 'static,
    {
        ...
        unsafe extern "C" fn text_to_glyphs_trampoline(
            ...
            utf8_len: std::ffi::c_int,
            glyphs: *mut *mut ffi::cairo_glyph_t,
            num_glyphs: *mut std::ffi::c_int,
