use crate::{
    draw::{
        ...
        mask::SubMaskRef,
        pipeline::{RasterPipelineBlitter, RasterPipelineBuilder},
        pixmap::SubPixmapMut,
        scan, *,
    },
    path::{PathStroker, Scalar, SCALAR_MAX},
};
...
impl PixmapMut<'_> {
    ...
    pub fn stroke_path(
        ...
        if stroke.width < 0.0 {
            eprintln!("negative stroke width isn't allowed");
            return;
        }
    ...
    pub fn apply_mask(&mut self, mask: &Mask) {
        if self.size() != mask.size() {
            eprintln!("Pixmap and Mask are expected to have the same size");
            return;
        }
