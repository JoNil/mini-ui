...
use crate::{
    draw::{
        geom::ScreenIntRect, pixmap::SubPixmapMut, wide::u32x8, Color, PixmapRef,
        PremultipliedColor, PremultipliedColorU8, SpreadMode, Transform,
    },
    path::NormalizedF32,
};
...
impl RasterPipelineBuilder {
    ...
    pub fn compile(self) -> RasterPipeline {
        ...
        if self.force_hq_pipeline || !is_lowp_compatible {
            let mut functions: Vec<_, MAX_STAGES> = self
                .stages
                .iter()
...
pub enum RasterPipelineKind {
    High {
        functions: Vec<highp::StageFn, MAX_STAGES>,
        tail_functions: Vec<highp::StageFn, MAX_STAGES>,
    },
    Low {
        functions: Vec<lowp::StageFn, MAX_STAGES>,
        tail_functions: Vec<lowp::StageFn, MAX_STAGES>,
    },
}
...
mod blend_tests {
    ...
    use super::*;
    use crate::drwa::{BlendMode, Color, Pixmap, PremultipliedColorU8};
    use crate::drwa::geom::IntSizeExt;
