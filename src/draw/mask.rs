use crate::{
    draw::{
        geom::IntSizeExt, painter::DrawTiler, pipeline::RasterPipelineBlitter,
        pixmap::SubPixmapMut, scan, FillRule, PixmapRef,
    },
    path::{IntRect, IntSize, Path, Scalar, Transform},
};
...
impl Mask {
    ...
    pub fn fill_path(
        ...
        if transform.is_identity() {
            ...
            if path_bounds.width().is_nearly_zero() || path_bounds.height().is_nearly_zero() {
                eprintln!("empty paths and horizontal/vertical lines cannot be filled");
                return;
            }

            if crate::draw::painter::is_too_big_for_math(path) {
                eprintln!("path coordinates are too big");
                return;
            }
            ...
            if let Some(tiler) = DrawTiler::new(self.width(), self.height()) {
                ...
                for tile in tiler {
                    ...
                    path = match path.transform(ts) {
                        Some(v) => v,
                        None => {
                            eprintln!("path transformation failed");
                            return;
                        }
                    };
    ...
    pub fn intersect_path(
        ...
        for (a, b) in self.data.iter_mut().zip(submask.data.iter()) {
            *a = crate::draw::color::premultiply_u8(*a, *b);
        }
    }
...
impl<'a> SubMaskRef<'a> {
    pub(crate) fn mask_ctx(&self) -> crate::draw::pipeline::MaskCtx<'a> {
        crate::draw::pipeline::MaskCtx {
            data: self.data,
