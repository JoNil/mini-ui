use crate::path::{
    floating_point::{FiniteF32, NonZeroPositiveF32, NormalizedF32, NormalizedF32Exclusive},
    path::{PathSegment, PathSegmentsIter, PathVerb},
...
fn compute_pos_tan(
    ...
    match seg_kind {
        ...
        SegmentType::Quad => {
            let src = &points[0..3].try_into().unwrap();
            if let Some(pos) = pos {
        ...
        SegmentType::Cubic => {
            let src = &points[0..4].try_into().unwrap();
            if let Some(pos) = pos {
